...
namespace System.Windows.Forms.DataVisualization.Charting
{
	public class ChartTool_HorizontalEqualLine : ChartTool, IMouseEventHandler, IAnnotationEventHandler, IAnnotationEditor
	{
		...
		public bool OnMouseMove(MouseEventArgs e)
		{
			...
			foreach (var anot in lst)
			{
				try
				{
					double x = curAnot.AxisX.PixelPositionToValue(e.X) - curAnot.X;

					if (-11 < x && x < 11) x = 11 * (x >= 0 ? 1 : -1);
		...
		public void AnnotationPaint(Annotation anot, PaintEventArgs e)
		{
			...
			{
				double benchmarkX = anot.AxisX.ValueToPixelPosition(anot.X);
				double x = anot.AxisX.ValueToPixelPosition(anot.Width + anot.X);

				if (x - benchmarkX >= 0)
				{
					x -= (this.LabelOffsetX + 75f);
				}
				else
				{
					x += this.LabelOffsetX;
